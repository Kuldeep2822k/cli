...
`palee validate` runs a ten-rule validation framework (rules live under [src/validation/rules/](https://github.com/Kuldeep2822k/cli/blob/main/src/validation/rules/), registered in `src/cli/validate.ts`, exported through the [src/validation/](https://github.com/Kuldeep2822k/cli/blob/main/src/validation/index.ts) barrel): three error-severity graph integrity rules ported from the dependency engine, five error-severity schema, identity, and assessment rules, plus three warning-severity rules — the two snapshot rules that explain gaps in the collected topic set, and the mastery-drift rule that keeps stored derived data honest.
