...
| **Auto-Chain Ordering (INV-46/47)** | `adopt --auto-chain[=strict\|toc\|full]` chains from Tier-0 hygiene over the numbered tree — inside the hygiene planner a directory leads with its README-class doc, then numeric lessons, then `deep-dive → lab → exam`, then remaining docs, with homework (`assignment/quiz/solution`) last, and an alphabetical transition between unnumbered sibling directories never gates (each such directory opens its own chain; the public `compareLessonOrder` shipped by Work Order A is unchanged) — bridging numbered modules and, by owner ruling PAL-205-G2, the vault-root README into the first numbered module (the only root exception; root into an unnumbered directory still refuses); the `toc`/`full` tiers additionally chain what the numbering does not cover from the repo's own `README`/`SUMMARY` markdown-link enumeration (fail-closed resolution, cycle-free by construction, same-dir resort mirroring the hygiene ranks), with numbering dominance meaning numbered-tree endpoints always follow numeric order; edges record their tier in the additive `depends_on_source: numbered\|toc` field; with no order signal anywhere the CLI reports `0 edges (no numbered layout, no README TOC links) — consider palee roadmap` and exits 0 instead of chaining alphabetically; `roadmap --auto-chain` orders by `order` (unordered keep file order, appended last, and an edge that would close a cycle against an authored dependency is skipped with a warning rather than failing the import). A numeric prefix counts only when a separator or the end of the name follows it and spans at most three digits, so `3d-printing.md` and `2024-recap.md` are ordinary unnumbered notes rather than lessons 3 and 2024. The adopt plan spans every note in the scanned scope — the notes being adopted *plus* those already adopted there — so a chain bridges onto an already-adopted note instead of restarting at the first new one: that note supplies its existing `palee_id` as the predecessor and is never rewritten, and `--dry-run` previews exactly the edges the commit writes. Notes filtered out by `--include`/`--exclude`/`--tag` join no plan and are stepped over. Both commands validate the planned graph for cycles before any write, reporting cycles by vault-relative path and labeling edges the flag synthesized; explicit non-empty `depends_on` always wins. |
