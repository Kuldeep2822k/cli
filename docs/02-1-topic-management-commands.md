...
1. **Ordering**: Notes are grouped by immediate parent directory. Within each group, notes sort by numeric filename prefix, then `deep-dive` → `lab` → `exam`, then alphabetically. Directory groups sort by numeric prefix; unnumbered names sort alphabetically after numbered ones at the same segment level. The alphabetical warning is raised only for unnumbered names that actually participate in such a comparison — a note whose basename has no lesson prefix, or an unnumbered directory group sharing a segment level with a different group — so a lone shared container like `MODULES/` does not trigger it on its own. A numeric prefix must be followed by a separator (`-`, `_`, `.`, space) or end the name, and is at most three digits: `01-intro.md` and `7.md` are lessons 1 and 7, while `3d-printing.md`, `01foundations.md` and `2024-recap.md` are not lesson numbers and chain as unnumbered notes.
