...
- **Memory-subsystem validation rules (#27, #41, #42, #44)**: `palee validate` now runs four rules on the memory subsystem, collected in the same single-read snapshot as the topic vault — `valid-managed-note-kind` (a note declaring `palee_schema` must carry exactly one recognized identity: `palee_id`/`session_id`/`memory_id`/`type: session_index`; no identity or conflicting kinds warn), `valid-session-schema` (canonical session notes need the required fields, `completed`/`draft` status coherent with the `S-`/`DRAFT-S-` filename convention, `session_id` matching the filename stem, and ordered parseable ISO timestamps), `no-session-unknown-topic` (a session's `topic_id` must reference an existing topic — phantom `T-general` included, no special case; `--strict` gates), and `valid-session-index` (the derived `.palee/index.md` must parse and its session references must point at existing confirmed sessions; findings never gate — the index is a rebuildable projection). A fresh vault with no memory subsystem validates clean. ([#27](https://github.com/Kuldeep2822k/cli/issues/27), [#41](https://github.com/Kuldeep2822k/cli/issues/41), [#42](https://github.com/Kuldeep2822k/cli/issues/42), [#44](https://github.com/Kuldeep2822k/cli/issues/44), [#168](https://github.com/Kuldeep2822k/cli/pull/168))
