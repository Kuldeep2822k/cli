...
| `palee validate` | `valid`, `topic_count`, `file_count`, `error_count`, `warning_count`, `errors[]`, `warnings[]` | Vault graph integrity and schema validation report. | `valid: boolean`, `errors: [{ type, rule_id, severity, message, id, topic, missing, path, files }]`, `warnings: [{ type, rule_id, severity, message, file, details }]` |
