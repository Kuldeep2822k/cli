...
| Topic Resolution | Planned | Advanced matching: exact ID → title → slug → token distance [planning/invariants.md#42](https://github.com/Kuldeep2822k/cli/blob/main/planning/invariants.md?plain=1#L42-L42) | `src/engine/index.ts` |
| Transactional Fixes | Planned | General fix engine for `validate --fix` (broken dependencies, missing fields) — the scoped SM-2 `valid-review-fields` repair already shipped (BUG-003); remaining classes still modeled via `fixable` rule metadata, deferred per [ADR-0008](https://github.com/Kuldeep2822k/cli/blob/main/docs/adr/0008-validation-framework-decisions.md) | `validate.ts` |

Sources:[planning/PHASE_2_GAPS.md#1-110](https://github.com/Kuldeep2822k/cli/blob/main/planning/PHASE_2_GAPS.md?plain=1#L1-L110)[docs/adr/0008-validation-framework-decisions.md](https://github.com/Kuldeep2822k/cli/blob/main/docs/adr/0008-validation-framework-decisions.md)