...
| `--auto-chain [tier]` | `boolean\|string` | `false` | Batch-only: derive each note's `depends_on` from the numbered tree and (for `toc`/`full`) the repo's README/SUMMARY enumeration (see §4). `strict`\|`toc`\|`full`, default `full`; bare flag = `full`, unknown value exits `2`. Conflicts with `--depends-on` and single-file mode. | `palee adopt "MODULES" --auto-chain -y` |
...
2. **Ordering**: Notes are grouped by immediate parent directory. Inside the hygiene planner a directory leads with its README-class doc, then sorts by numeric filename prefix, then `deep-dive` → `lab` → `exam`, then remaining docs, with homework (`assignment`/`quiz`/`solution`) deliberately LAST so it never gates the lessons that follow (PAL-205-C1, landed jointly with the PAL-205-B rework); an alphabetical transition between unnumbered sibling directories no longer gates at all — each such directory opens its own chain — while numbered cross-module bridges are kept. The public `compareLessonOrder` shipped with Work Order A is intentionally unchanged. Directory groups sort by numeric prefix; unnumbered names sort alphabetically after numbered ones at the same segment level. The alphabetical warning is raised only for unnumbered names that actually participate in such a comparison — a note whose basename has no lesson prefix, or an unnumbered directory group sharing a segment level with a different group — so a lone shared container like `MODULES/` does not trigger it on its own. A numeric prefix must be followed by a separator (`-`, `_`, `.`, space) or end the name, and is at most three digits: `01-intro.md` and `7.md` are lessons 1 and 7, while `3d-printing.md`, `01foundations.md` and `2024-recap.md` are not lesson numbers and chain as unnumbered notes.
3. **Chaining**: Each note depends on its predecessor in the ordered list; the first note of a module depends on the last note of the previous module only when the cross-directory transition is justified by structure — a numbered module bridge, a nesting relation, or the owner-ruled vault-root bridge (the root README gates into the first numbered module, PAL-205-G2; a root note reaching an unnumbered directory still refuses) (item 2). Alphabetical order between unnumbered sibling directories never bridges: each such directory opens its own chain. Excluded or already-adopted notes are bridged over, never rewritten.
4. **TOC tier (PAL-205-C, `--auto-chain=toc|full`)**: Notes the numbered tree does not cover are chained from the repo's own enumeration — `README.md`/`SUMMARY.md` documents at the vault root and inside visible, non-phase directories [src/storage/toc.ts](https://github.com/Kuldeep2822k/cli/blob/main/src/storage/toc.ts). Markdown links in document order become the chain (folder links → `<dir>/README.md`, trailing slashes deduped, anchors stripped, `%20` decoded with no raw fallback, `<angle bracket paths>` parsed, escapes rejected via `isWithinVault`, ambiguous/missing targets skipped fail-closed and counted, backslashes literal). Numbering dominance: where both endpoints live in the numbered tree, numbering wins even if a README enumerates otherwise. TOC chains are cycle-free by construction, re-verified in code. Edges record their author in the additive optional `depends_on_source: numbered|toc` frontmatter field (old builds ignore it; gating is unchanged this release).
5. **Tiers and honest refusal**: `--auto-chain=strict` chains only the numbered tree (TOC is never consumed); bare `--auto-chain` equals `full`; any other value exits `2`. `toc` and `full` currently produce identical plans — both chain the unnumbered remainder from the repo's own enumeration — and only `strict` differs. When the scope has no numbered layout, its scoped README/SUMMARY enumeration resolves no TOC link at all, and the plan writes zero edges, the CLI prints `0 edges (no numbered layout, no README TOC links) — consider palee roadmap` and exits `0` rather than fabricating prerequisites; a README that does enumerate its notes keeps their justified numbered edges (PAL-205-C rework), so the refusal never fires on a real enumeration. `strict` declining a TOC that does exist is a configuration choice and does not print the roadmap pointer.
...
7. **Conflicts**: `--auto-chain` conflicts with `--depends-on` and with single-file mode (both exit `2`). `--dry-run` prints exactly the edges the commit will write — notes already adopted in scope are listed separately as bridged predecessors, never as edges — and writes nothing. Both the dry-run and the confirmation screen print the Tier-0 hygiene block: backbone count, leaf count, skipped-by-rule counts (meta / translations / template), phase-subtree collapses, the invalid-`palee_id` count when non-zero, and the excluded total; when hygiene leaves nothing to chain, the block closes with a pointer to direct adoption. The `Auto-chain:` line names the selected tier and, when the TOC tier contributed, the number of TOC-authored edges. TOC resolution also records every skipped link with its reason (`escaped-vault` / `missing` / `ambiguous`) in `TocEnumeration.skipped` [src/storage/toc.ts](https://github.com/Kuldeep2822k/cli/blob/main/src/storage/toc.ts) — these counts are currently plan-data only and are printed on no CLI screen, so "skipped fail-closed and counted" (item 4) is verifiable in the API but not yet visible to the learner; surfacing them in `--verbose` is an open follow-up.

**Known issues (documented, non-blocking):**
- *Cosmetic warning seam (PAL-205-B6)*: the "…are not numbered lessons and chain in alphabetical order" warning is computed from the numbered plan and still prints immediately above a true-refusal `0 edges` line, where no alphabetical chaining in fact occurred. Display-order guard only; the edge plan and exit code are unaffected.
- *Badge-wrapped TOC links*: a link whose label contains an image (`[![x](i.png)](target.md)`) mis-parses its destination and the link is counted `missing` — fail-closed, never a false edge; it only matters for vault TOCs that use badge-links, in which case the enumeration silently loses that entry (visible in the skipped counts above).
