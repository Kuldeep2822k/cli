...
- **Hot-memory, safe-vault-paths, and diagnostic-value validation rules (#43, #45, #166)**: two rules complete the #25 rule backlog, plus a diagnostics fix. `valid-hot-memory` (#43, warning) validates `.palee/hot.md` as a rebuildable derived view: it must carry the writer's identity (`memory_id: H-active`), stay within the 250-word body cap (`MAX_HOT_WORDS`, counted with the same `countWords` the writer's truncation uses, frontmatter excluded), and its `last_session`/`active_topic` references must point at existing notes — reference checks are suppressed on a read-incomplete snapshot, and a missing or unparsable hot memory is rebuilt rather than diagnosed. `safe-vault-paths` (#45, error) audits every collected managed path — topic notes, scanned notes, and session notes — against the configured vault boundary: paths normalize (`\` → `/`) before validation, and traversal (`../`, including mid-path), absolute POSIX, and Windows drive-path escapes report as errors, mirroring the walker/roadmap containment policy. Diagnostics (#166, fix): non-finite frontmatter values (`NaN`/`Infinity`, YAML `.nan`/`.inf`) no longer render as `null` in findings — the `displayValue` helper is hoisted to a shared module and applied at every message and `details.actual` site (`valid-review-dates`, `valid-dependency-list`, `valid-assessment-fields`, `valid-topic-mastery`), so "must be null … got null" self-contradictions become the explicit `got "NaN"`. Nineteen rules registered; 27 new tests. ([#43](https://github.com/Kuldeep2822k/cli/issues/43), [#45](https://github.com/Kuldeep2822k/cli/issues/45), [#166](https://github.com/Kuldeep2822k/cli/issues/166), closes #43, closes #45, closes #166)

- **Memory-subsystem validation rules (#27, #41, #42, #44)**: `palee validate` now runs four rules on the memory subsystem, collected in the same single-read snapshot as the topic vault — `valid-managed-note-kind` (a note declaring `palee_schema` must carry exactly one recognized identity: `palee_id`/`session_id`/`memory_id`/`type: session_index`; no identity or conflicting kinds warn — including conflicts on the internal `.palee` notes the walker never sees), `valid-session-schema` (canonical session notes need the required fields, a supported `palee_schema` version, `completed`/`draft` status coherent with the `S-`/`DRAFT-S-` filename convention, `session_id` matching the filename stem, and ordered ISO 8601 timestamps with an explicit timezone designator; offset forms pass; date-only and timezone-less strings fail), `no-session-unknown-topic` (a session's `topic_id` must reference an existing topic — phantom `T-general` included, no special case; sessions failing the schema rule are skipped so findings never double-report; `--strict` gates), and `valid-session-index` (the derived `.palee/index.md` must parse and its session references must point at existing confirmed sessions; only session-shaped `[[S-…]]`/`[[DRAFT-S-…]]` links count — topic or note-title links in the editable body are never session refs; findings never gate — the index is a rebuildable projection). Read failures anywhere in the snapshot — a locked or deleted session note, an unlistable sessions directory, an unreadable `index.md`/`hot.md` — are retained and reported by `read-failure` and mark the scan provisional, so a partial memory snapshot can never silently validate clean. A fresh vault with no memory subsystem validates clean. ([#27](https://github.com/Kuldeep2822k/cli/issues/27), [#41](https://github.com/Kuldeep2822k/cli/issues/41), [#42](https://github.com/Kuldeep2822k/cli/issues/42), [#44](https://github.com/Kuldeep2822k/cli/issues/44), [#168](https://github.com/Kuldeep2822k/cli/pull/168))

- **Review-state, review-dates, and dependency-list validation rules (#33, #38, #39) + missing-dependency severity flip (#34)**: `palee validate` now runs three raw-frontmatter rules — `valid-review-fields` (SM-2 state as stored must match the engine contract: `ease_factor >= 1.3`, `interval_days >= 1`, counters `>= 0`, `last_quality` null or integer 0–5), `valid-review-dates` (`last_reviewed_at`/`due_at` must be null or strict real `YYYY-MM-DD` calendar dates, with inverted pairs reported), and `valid-dependency-list` (`depends_on` must be an array of non-empty string IDs; shape errors and self-references are errors, duplicates are warnings). Per ADR-0008 decision 1, `no-missing-dependency` findings are now warnings in vault scans — the engine quarantines the dependent topic, `roadmap --from` keeps its separate hard-error path, and `--strict` escalates. ([#33](https://github.com/Kuldeep2822k/cli/issues/33), [#34](https://github.com/Kuldeep2822k/cli/issues/34), [#38](https://github.com/Kuldeep2822k/cli/issues/38), [#39](https://github.com/Kuldeep2822k/cli/issues/39), [#165](https://github.com/Kuldeep2822k/cli/pull/165))
...
- **Schema, topic-id-format, and status validation rules (#28, #29, #31)**: three new `palee validate` rules — every PALEE-managed note must declare `palee_schema: 1` (unknown future versions error), topic IDs must match the centralized `T-` lowercase kebab-case policy (the pre-#29 adopt-generated `T-YYYYMMDDTHHMMSS-hex8` format remains valid; legacy snake-case IDs are reported as errors), and `status` must be one of the four documented states. `adopt` now generates policy-compliant IDs. ([#28](https://github.com/Kuldeep2822k/cli/issues/28), [#29](https://github.com/Kuldeep2822k/cli/issues/29), [#31](https://github.com/Kuldeep2822k/cli/issues/31), [#159](https://github.com/Kuldeep2822k/cli/pull/159))
...
- **ADR-0008: validation-framework decisions recorded as a permanent ADR**: the four product decisions that governed the #25 rule backlog (missing-dependency severity = warning in vault scans, difficulty = string enum, warnings-only exits 0 unless `--strict`, derived views never gate) plus the framework's architecture decisions (pure visitor rules, single-read snapshot, raw-frontmatter reads, fixability metadata) now live in [docs/adr/0008-validation-framework-decisions.md](https://github.com/Kuldeep2822k/cli/blob/main/docs/adr/0008-validation-framework-decisions.md) instead of only the removed `planning/VALIDATION_FRAMEWORK_VERDICT.md`. All dangling references to that deleted planning doc (docs indexes, `format.ts` comment, CHANGELOG, code comments, and `02-3-reporting-commands.md` "VERDICT decision" citations) now cite ADR-0008; the same pass cleaned dead links to the other removed planning docs (`PHASE_1_ISSUES.md`, `PHASE_1_CHECKLIST.md`, `TRIGGER_TRACKER.md`) from `agent.md` and the docs pages. ([#170](https://github.com/Kuldeep2822k/cli/issues/170), closes #170)
