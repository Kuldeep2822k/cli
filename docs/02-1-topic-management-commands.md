...
2. **Ordering**: Notes are grouped by immediate parent directory. Within each group, notes sort README-class first, then by numeric filename prefix, then `deep-dive` → `lab` → `exam`, then homework (`assignment`/`quiz`/`solution`, always last so it never gates lessons), then alphabetically (PAL-205-C1). Directory groups sort by numeric prefix; phase directories rank `start` → `sketch` → `your-work` → `answers` → `solution(s)` after lesson directories; unnumbered directories/files sort alphabetically after numbered ones and emit a warning. A numeric prefix must be followed by a separator (`-`, `_`, `.`, space) or end the name, and is at most three digits: `01-intro.md` and `7.md` are lessons 1 and 7, while `3d-printing.md`, `01foundations.md` and `2024-recap.md` are not lesson numbers and chain as unnumbered notes.
...
5. **Tiers and honest refusal**: `--auto-chain=strict` chains only the numbered tree (TOC is never consumed); bare `--auto-chain` equals `full`; any other value exits `2`. When the scope has no numbered layout and no README TOC links, the CLI prints `0 edges (no numbered layout, no README TOC links) — consider palee roadmap` and exits `0` rather than fabricating prerequisites.
...
7. **Conflicts**: `--auto-chain` conflicts with `--depends-on` and with single-file mode (both exit `2`). `--dry-run` prints exactly the edges the commit will write — notes already adopted in scope are listed separately as bridged predecessors, never as edges — and writes nothing. Both the dry-run and the confirmation screen print the Tier-0 hygiene block: backbone count, leaf count, skipped-by-rule counts (meta / translations / template), phase-subtree collapses, and the excluded total.
