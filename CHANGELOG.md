...
- **Assessment and mastery validation rules (#36, #37) + assessment-review independence coverage (#40)**: `palee validate` now also runs `valid-assessment-fields` (#36, error): the four assessment scores (`conceptual`, `practical`, `debug`, `feynman`) must be finite numbers within `[0.0, 1.0]` as stored on disk — the rule reads raw frontmatter values, because the loader clamps and coerces during normalization, so real vault corruption is exposed instead of silently blessed — and `assessed_at` must be `null` or a parseable date; missing assessment fields follow the documented adopt-default policy and pass. `valid-topic-mastery` (#37, warning): when a topic's assessment fields are shape-valid, stored `topic_mastery` must equal the engine formula `round((conceptual + practical + debug + 2*feynman) / 5, 4)` recomputed via `computeTopicMastery` (the same engine every runtime consumer uses); drift reports `details.actual`/`details.expected`, topics failing #36 are skipped (no double-reporting), missing assessment data never crashes the rule, archived topics are still checked, and the finding is a warning so `--strict` gates it. Both rules are registered in the validate catalog and exported through the validation barrel. #40 (assessment-review independence) is enforced as command-level regression tests per the issue design rather than a static vault rule: `test/e2e/assessment-review-independence.test.ts` runs the real CLI and asserts `palee review` updates only SM-2 fields while assessment data — including a non-zero `topic_mastery` — survives byte-for-byte, across repeated reviews; the enforcement mechanism is documented in the reporting-commands doc.

- **Validation barrel + `--strict` warning escalation (#25)**: The `src/validation/` subsystem now ships a public barrel (`src/validation/index.ts`, matching the storage barrel convention) exporting the framework surface — `collectVault`, `runRules`, `formatHuman`/`formatJson`, all eight registered rules, and the shared contract types — re-exported at the package root (`export * from './validation'`) so npm consumers can import it; the surface is pinned by a census test modeled on `test/storage-barrel-census.test.ts`. `palee validate` gains `--strict`: a warnings-only vault still exits `0` by default (adopted severity policy unchanged), but exits `3` with `--strict` — the same zero-tolerance stance other linters express through `--max-warnings 0`-style flags — in both human and JSON modes. The JSON contract is untouched (`valid` still reflects errors only; `--strict` escalates the exit code, never the payload), errors always exit `3` with or without `--strict`, and a clean vault stays `0` under strict.

- **Three schema and identity validation rules (#28, #29, #31)**: `palee validate` now also runs `valid-palee-schema` (every PALEE-managed note — topic, session, hot memory, session index — must declare `palee_schema: 1`; missing, non-integer, and future versions are errors, so migrations and mutations can refuse to guess at unknown data; non-managed user notes are never reported), `valid-topic-id-format` (topic IDs must match the centralized policy in the new `src/engine/topic-id.ts`: `T-` plus lowercase kebab-case segments — `T-git-rebase`; the exact legacy adopt-generated format `T-YYYYMMDDTHHMMSS-hex8` remains valid so existing vaults do not fail, legacy snake-case IDs are reported without mutation), and `valid-topic-status` (status must be one of `not_started` | `learning` | `paused` | `archived`; pseudo-statuses like `completed`/`done` are errors because mastery is derived, never stored; missing status is tolerated as the adopt default, so adopt-style notes without a status key validate clean). `adopt` now generates policy-compliant IDs (`T-YYYYMMDD-HHMMSS-hex8`, all-lowercase segments) instead of the uppercase-T timestamp form.

- **Validation rule framework with first rule catalog**: `palee validate` now runs on a rule framework (`src/validation/`: types, vault collector, deterministic runner, human/JSON formatters) instead of hardcoded checks inside the command handler. The existing duplicate-ID, missing-dependency, and cycle checks are preserved as pure rules (`no-duplicate-topic-id`, `no-missing-dependency`, `no-dependency-cycle`) that delegate to the engine — findings, JSON contract, and exit codes unchanged; human wording differs slightly (bullet text now carries the rule id, e.g. `Missing dependency: X depends on Y` → `Topic X depends on missing topic Y`) and malformed YAML frontmatter is now reported as a `parse-frontmatter` warning instead of silently skipping the note (invariant: one bad note never aborts a vault scan; unclosed `---` fences are detected when the fence body reads like YAML — key/value pairs, sequences at any indent — while a leading `---` thematic break is left alone). Collection reads each file exactly once and threads those bytes into both the parse outcomes and topic loading, so a note edited mid-scan cannot make warnings and graph rules observe different versions of the vault; unreadable files are excluded from the graph but kept in the scan report as a `read-failure` warning — graph findings stay deterministic error-severity regardless (an unrelated locked file can never hide a genuine missing-dependency error), and re-running validation settles any transient condition. Warnings never gate the exit code (warnings-only runs exit `0`, errors exit `3`), and `palee validate --json` keeps the documented contract (`valid`, `topic_count`, `file_count`, `error_count`, `errors[]`) while adding `warning_count`/`warnings[]` with `rule_id`/`severity`/`message` on every entry ([#25](https://github.com/Kuldeep2822k/cli/issues/25), closes #26, closes #30).

- **Multi-cycle enumeration + quarantine in the dependency engine**: `detectCycles` (new) enumerates every distinct cycle in the topic graph (the old `detectCycle` was a two-color scan that returned only the first cycle and aborted), reporting each cycle's exact path in canonical rotation (lexicographically-smallest ID leads) — including one-node cycles from self-referential `depends_on` entries and overlapping cycles that share nodes or back edges. The enumeration is two-stage and recursion-free — iterative Tarjan SCC decomposition, then Johnson-style elementary-cycle enumeration within each cyclic SCC (with Johnson's deferred unblocking, so enumeration cost stays proportional to the cycles reported rather than to the number of simple paths) — so deep dependency chains and large strongly connected components can neither overflow the call stack nor stall `plan`. The returned cycle list is sorted by canonical path, stable regardless of vault insertion order. `quarantineCyclicTopics` (new) removes every topic on or transitively depending on a cycle from the working graph (single O(V+E) reverse-reachability pass — acyclic sibling prerequisites of a cyclic dep stay usable) and returns the clean acyclic subgraph plus the cycle paths, so `plan` now continues on valid acyclic components per the spec — quarantined cycles are surfaced as a human-readable section and a `quarantined_cycles` array + `counts.quarantined` field (JSON mode, present on empty vaults too). Quarantine membership is derived from SCC analysis (`findCyclicSccNodes`), so it is O(V+E) and provably independent of cycle enumeration; `plan` displays a bounded sample via `detectCyclesBounded` (default 1000 cycles) with a `truncated` flag and a `quarantined_cycles_truncated` JSON field — dense-but-valid dependency SCCs (which contain exponentially many elementary cycles) can no longer make `plan` spend unbounded time before producing output, and truncation can never under-quarantine. `validateDependencyGraph` reports one error per distinct cycle instead of stopping at the first. `getReadyTopics` output is now deterministically ordered by ascending `palee_id` (code-unit comparison, locale-independent), independent of vault-walk insertion order. `detectCycle` keeps its signature and first-cycle-or-`null` semantics as a compatibility wrapper ([#79](https://github.com/Kuldeep2822k/cli/issues/79)).
...
- **Vault-relative paths stay clean through symlinked vault roots**: `walkVault` resolves the vault root via `realpathSync` (#122), so walked file paths carry the canonical prefix while consumers computed `path.relative` against the unresolved caller-supplied root — producing escaping garbage (`../../private/var/...`) whenever the vault root itself is reached through a symlink, which is the default for macOS temp dirs (`/var/folders/...` → `/private/var/folders/...`) and broke every walk-based test on the macOS CI matrix (main-only; the PR matrix runs ubuntu/windows). New `relativeVaultPath()` helper (`src/storage/vault-walker.ts`, exported through the storage barrel) computes the lexical relative path first and re-derives it from both endpoints canonicalized only when the lexical path escapes the root — non-symlinked filesystems pay no extra syscall, and files genuinely outside the vault keep their truthful escaping path. Wired through `scanNotes`, `loadTopics` (topic `path`), and `adopt` batch scanning; pinned by symlinked-root regression tests (junction on win32, dir elsewhere) asserting `topic.md`/`sub/note.md` come out exactly clean through scanner, loader, and `collectVault` ([#160](https://github.com/Kuldeep2822k/cli/issues/160)).
