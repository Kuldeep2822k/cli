...
- Topic: The primary unit of learning, containing metadata, mastery status, and scheduling data [[src/types.ts:49-59]]().
- Assessment: A four-pillar model (Conceptual, Practical, Debug, Feynman) used to track mastery levels [[src/types.ts:3-9]]().
- Review: Contains the Spaced Repetition System (SRS) state, including ease factor and next due date [[src/types.ts:11-19]]().
...
PALEE uses a strict `Difficulty` union type: `'beginner' | 'intermediate' | 'advanced'`[[src/types.ts:21-21]](). To handle varied user input from the CLI or Markdown frontmatter, the system provides a `normalizeDifficulty` utility.
...
- SessionRecord: Records the lifecycle of a learning session (start, end, status) [[src/types.ts:86-93]]().
...
- PaleeConfig: Defines the user's local environment settings, such as `vaultPath` and AI preferences [[src/types.ts:104-108]]().
- CacheEntry: The schema for the file system cache, including `mtime` and `fingerprint` for validation [[src/types.ts:122-128]]().
- ValidationResult: The output of vault integrity checks, listing `ValidationError` types like `cycle` or `missing_dependency`[[src/types.ts:142-155]]().
- CLI Options: Specific interfaces for command arguments, such as `AdoptOptions`, `NextOptions`, and `PlanOptions`[[src/types.ts:168-206]]().
