...
- **Tiered chain sources with a TOC tier** ([PAL-205-C](https://github.com/Kuldeep2822k/cli/issues/73), on top of Tier-0 hygiene PAL-205-B):
  ...
  - **C1 phase ordering in the numbered backbone**: README-class docs now chain first inside a lesson dir, then numeric lessons, then `deep-dive → lab → exam`, then `assignment/quiz/solution` LAST (homework no longer gates lessons); at directory level, phase dirs rank `start → sketch → your-work → answers → solution(s)` and sort after lesson dirs. The alphabetical-fallback warning narrows to genuinely ad-hoc names.
