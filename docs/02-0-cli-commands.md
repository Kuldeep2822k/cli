...
| :---: | :--- | :--- | :--- |
| **0** | **Success / Clean Exit** | Command completed successfully, help text was printed, dry-run simulation finished, or the user gracefully cancelled an interactive prompt. | Successful review, roadmap import complete, `palee validate` passed with 0 errors, `palee adopt --dry-run`. |
| **1** | **Partial Import Failure** | The operation completed partially, but one or more sub-items failed to write to disk. | `palee roadmap` where some notes in the curriculum failed atomic write (`failed > 0`). |
...
| **4** | **OCC Concurrency Conflict** | Optimistic Concurrency Control detected a mid-air collision (`ECONFLICT` / `isConflictError`). A note's SHA-256 fingerprint changed between reading and writing. | Concurrent edits by Obsidian or external scripts during `review`, `adopt`, `roadmap`, or `session`. |
...
| `palee review` | Successfully updated SM-2 interval, ease factor, repetition count, and `due_at`. | N/A | Quality rating not an integer `0..5`, unconfigured vault, topic not found, or ambiguous query (multiple matches). | N/A | Target note modified concurrently during review (`isConflictError`). | Atomic write failure or unexpected file system error. |
...
| `palee roadmap` | Curriculum parsed, validated, and all topics created/updated on disk. | Partial import failure (`failed > 0` topic notes failed to write). | Missing `--from` argument, roadmap file not found, malformed structure (missing `topics`), or non-interactive stdin without `-y`. | Roadmap validation failed (missing ID/title/path, duplicate ID/path, invalid difficulty/order, missing dependency, cycle detected). | OCC conflict during atomic note write. | Unexpected runtime / I/O exception. |
...
| `palee session` | Session lifecycle action (`start`, `draft`, `end`, `list`) completed successfully. | N/A | Unconfigured vault, missing `--topic` for `draft`/`end` when no active topic exists, or unknown action. | N/A | OCC conflict during session note write or `hot.md` update. | Unexpected runtime exception or directory write failure. |
