...
| `palee session` | Session lifecycle action (`start`, `draft`, `end`, `list`) completed successfully. | N/A | Unconfigured vault, missing `--topic` for `draft`/`end` when no active topic exists, unconfirmed drafts blocking non-interactive `session start`, an interactive draft menu that reaches end of input before every draft is answered, or unknown action. | N/A | OCC conflict during session note write or `hot.md` update (`isConflictError`). | Unexpected runtime exception or storage directory error. |
...
| `palee plan` | `reviews_due[]`, `ready_to_learn[]`, `quarantined_cycles[]`, `quarantined_cycles_truncated`, `counts`, `total_topics` | Topological daily study plan; dependency cycles are quarantined out of `ready_to_learn` with their exact cycle paths. | `quarantined_cycles_truncated: boolean` (true when more cycles exist than the 1000-cycle reporting cap), `counts: { due, ready, quarantined, mastered, learning, new }` |
...
| `palee validate` | `valid`, `topic_count`, `file_count`, `error_count`, `warning_count`, `errors[]`, `warnings[]` | Vault graph integrity and schema validation report. | `valid: boolean`, `errors/warnings: [{ type, rule_id, severity, message, file?, topic_id?, session_id?, field?, details? }]` with legacy locator keys preserved per rule (`id`+`files` for `duplicate_id`; `topic`+`missing` for `missing_dependency`; `path` for `cycle`) |
