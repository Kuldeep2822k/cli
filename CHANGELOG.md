...
- **Validation rule framework with first rule catalog**: `palee validate` now runs on a proper rule framework (`src/validation/`: types, vault collector, deterministic runner, human/JSON formatters) instead of hardcoded checks inside the command handler. The existing duplicate-ID, missing-dependency, and cycle checks are preserved as pure rules (`no-duplicate-topic-id`, `no-missing-dependency`, `no-dependency-cycle`) that delegate to the engine — behavior and exit codes unchanged — and malformed YAML frontmatter is now reported as a `parse-frontmatter` warning instead of silently skipping the note (invariant: one bad note never aborts a vault scan; unclosed `---` fences are also detected). Warnings never gate the exit code (warnings-only runs exit `0`, errors exit `3`), and `palee validate --json` keeps the documented contract (`valid`, `topic_count`, `file_count`, `error_count`, `errors[]`) while adding `warning_count`/`warnings[]` with `rule_id`/`severity`/`message` on every entry ([#25](https://github.com/Kuldeep2822k/cli/issues/25), closes #26, closes #30).
