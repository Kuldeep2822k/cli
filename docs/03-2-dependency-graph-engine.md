...
| **Auto-Chain Ordering (INV-46/47)** | `adopt --auto-chain` orders by numeric prefix → `deep-dive → lab → exam` → alphabetical, bridging modules; `roadmap --auto-chain` orders by `order` (unordered keep file order, appended last). The adopt plan spans every note in the scanned scope — the notes being adopted *plus* those already adopted there — so a chain bridges onto an already-adopted note instead of restarting at the first new one: that note supplies its existing `palee_id` as the predecessor and is never rewritten. Notes filtered out by `--include`/`--exclude`/`--tag` join no plan and are stepped over. Both commands validate the planned graph for cycles before any write; explicit non-empty `depends_on` always wins. |
